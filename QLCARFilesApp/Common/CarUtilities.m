...
NSString * const kCarInfoDict_NSImageKey = @"kCarInfoDict_NSImageKey";
NSString * const kCarInfoDict_UTITypeKey = @"kCarInfoDict_UTITypeKey";
NSString * const kCarInfoDict_DataKey = @"kCarInfoDict_DataKey";

//
// Helper functions for UTI type conversions
//
static NSString *GetFileExtensionFromUTI(NSString *utiType)
{
	if([utiType length] == 0)
	{
		return nil;
	...
	if (@available(macOS 11.0, *)) {
		...
		return CFBridgingRelease(UTTypeCopyPreferredTagWithClass((__bridge CFStringRef _Nonnull)(utiType), kUTTagClassFilenameExtension));
		#pragma clang diagnostic pop
	}
}

static NSString *GetUTIFromFileExtension(NSString *extension)
{
	...
	if (@available(macOS 11.0, *)) {
		...
		#pragma clang diagnostic push
		#pragma clang diagnostic ignored "-Wdeprecated-declarations"
		return CFBridgingRelease(UTTypeCreatePreferredIdentifierForTag(kUTTagClassFilenameExtension, (__bridge CFStringRef _Nonnull)(extension), nil));
		#pragma clang diagnostic pop
	}
}
	...
	[catalog enumerateNamedLookupsUsingBlock:^(CUINamedLookup *namedLookup)
	{
		@autoreleasepool
		{
			...
			if ([namedLookup isKindOfClass:[CUINamedImage class]])
			{
				...
				if(cgImage != NULL)
				{
					// Use the Swift-safe utility function that creates NSImage with its own copy of the data
					NSImage *nsImage = CreateNSImageFromCGImageForSwift(cgImage);
					if(nsImage != nil)
					{
						carNamedLookupDict[kCarInfoDict_NSImageKey] = nsImage;
					}
				}
			...
			else if([namedLookup isKindOfClass:[CUINamedData class]])
			{
				...
				// Update fileName using UTI helper functions
				NSString *fileName = renditionName;
				NSString *utiType = [asset utiType];

				if([utiType length] > 0)
				{
					NSString *preferredExtension = GetFileExtensionFromUTI(utiType);
				...
				else
				{
					...
					{
						utiType = GetUTIFromFileExtension(pathExtension);
					}
				}
			...
			else if([namedLookup isKindOfClass:[CUINamedColor class]])
			{
				...
				if(cgColor != NULL)
				{
					...
					{
						// Use the Swift-safe utility function that creates NSImage with its own copy of the data
						NSImage *nsImage = CreateNSImageFromCGImageForSwift(imageRef);
						CGImageRelease(imageRef); // Release the temporary CGImage
						if(nsImage != nil)
						{
							carNamedLookupDict[kCarInfoDict_NSImageKey] = nsImage;
						}
