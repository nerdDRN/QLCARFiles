...
//
// Creates a NSImage from a CGImageRef with proper memory management for Swift
// This function creates an NSImage that owns its own copy of the image data
//
NSImage *CreateNSImageFromCGImageForSwift(CGImageRef inCGImageRef)
{
	if(inCGImageRef == NULL)
	{
		return nil;
	...
	size_t width = CGImageGetWidth(inCGImageRef);
	size_t height = CGImageGetHeight(inCGImageRef);
	size_t bitsPerComponent = CGImageGetBitsPerComponent(inCGImageRef);

	// Create a new bitmap context and draw the image into it
	// This creates a copy of the image data
	CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
	CGContextRef context = CGBitmapContextCreate(nil, width, height, bitsPerComponent, 0, colorSpace, kCGImageAlphaPremultipliedLast);
	...
	CGContextDrawImage(context, CGRectMake(0, 0, width, height), inCGImageRef);

	// Create a new CGImage from the context (this has its own copy of the data)
	...
	// Create NSImage from the copied CGImage
	NSSize imageSize = NSMakeSize(width, height);
	NSImage *nsImage = [[NSImage alloc] initWithCGImage:copiedImage size:imageSize];

	// Force NSImage to create its own bitmap representation immediately
	// by locking and unlocking focus. This causes NSImage to render itself
	// into its own internal bitmap storage.
	[nsImage lockFocus];
	[nsImage unlockFocus];

	// Now we can safely release the copied image since NSImage has created its own representation
